  ...
  Future<void> _saveParkingSpot() async {
    ...
    try {
      ...
      for (int i = 1; i <= int.parse(_capacityController.text); i++) {
        // Generate a unique ID for each spot
        String spotId = FirebaseFirestore.instance.collection('random').doc().id;
        String spotNumber = 'SPOT_$i';
        
        // Prepare spot data for Realtime Database with new format
        spotsData[spotId] = {
          'spotId': spotId,
          'number': spotNumber,
          'status': 'available',
        ...
        // Add spot to Firestore batch with new format
